...
public class TChDirDialog : TDialog
{
    ...
    public TChDirDialog(ushort opts, ushort histId)
        : base(new TRect(16, 2, 64, 21), SharpVisionIntl.Get("ChDir_Title", "Change Directory"))
    {
        ...
        chDirButton = new TButton(new TRect(35, 9, 45, 11),
                                  SharpVisionIntl.Get("ChDir_Btn_Chdir", "~C~hdir"), Views.cmChangeDir, ButtonConstants.bfNormal);
        Insert(chDirButton);
        Insert(new TButton(new TRect(35, 12, 45, 14),
                           SharpVisionIntl.Get("ChDir_Btn_Revert", "~R~evert"), Views.cmRevert, ButtonConstants.bfNormal));
        if ((opts & ChDirDialogOptions.cdHelpButton) != 0)
            Insert(new TButton(new TRect(35, 15, 45, 17),
                               SharpVisionIntl.Get("Btn_Help", "~H~elp"), Views.cmHelp, ButtonConstants.bfNormal));
        ...
        catch (System.UnauthorizedAccessException ex)
        {
            ...
            if (owner != null)
                MsgBox.MessageBox(owner,
                    string.Format(
                        SharpVisionIntl.Get("File_Err_AccessDenied", "Access denied: '{0}'"),
                        target),
