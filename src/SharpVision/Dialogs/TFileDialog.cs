...
public class TFileDialog : TDialog, IFileDialogContext
{
        ...
        : base(new TRect(15, 1, 64, 21), aTitle)
    {
        ...
        Insert(fileList);
        Insert(new TLabel(new TRect(2, 4, 17, 5), SharpVisionIntl.Get("File_Label_Files", "~F~iles"), fileList));
        ...
        AddButton(FileDialogOptions.fdOpenButton,    SharpVisionIntl.Get("File_Btn_Open",    "~O~pen"),    Views.cmFileOpen);
        AddButton(FileDialogOptions.fdOKButton,      SharpVisionIntl.Get("Btn_OK",           "~O~K"),      Views.cmFileOpen);
        AddButton(FileDialogOptions.fdAddButton,     SharpVisionIntl.Get("File_Btn_Add",     "~A~dd"),     Views.cmFileOpen);
        AddButton(FileDialogOptions.fdSelectButton,  SharpVisionIntl.Get("File_Btn_Select",  "~S~elect"),  Views.cmFileSelect);
        AddButton(FileDialogOptions.fdReplaceButton, SharpVisionIntl.Get("File_Btn_Replace", "~R~eplace"), Views.cmFileReplace);
        AddButton(FileDialogOptions.fdClearButton,   SharpVisionIntl.Get("File_Btn_Clear",   "~C~lear"),   Views.cmFileClear);

        bt = new TButton(r,
            (aOptions & FileDialogOptions.fdDoneButton) != 0
                ? SharpVisionIntl.Get("Btn_Done", "Done")
                : SharpVisionIntl.Get("Btn_Cancel", "Cancel"),
        ...
        {
            bt = new TButton(r, SharpVisionIntl.Get("Btn_Help", "~H~elp"), Views.cmHelp, ButtonConstants.bfNormal);
            bt.growMode = (byte)(Views.gfGrowLoX | Views.gfGrowHiX);
            Insert(bt);
            r = new TRect(r.a.x, r.a.y + 2, r.b.x, r.b.y + 2);
        }
    ...
    public virtual bool CheckDirectory(string str)
    {
        ...
        catch (System.Exception ex)
        {
            if (owner != null)
                MsgBox.MessageBox(owner,
                    string.Format(SharpVisionIntl.Get(
        ...
        if (owner != null)
            MsgBox.MessageBox(owner,
                SharpVisionIntl.Get("File_Err_InvalidDir", "Invalid drive or directory."),
                MsgBox.mfError | MsgBox.mfOKButton);
        fileName?.Select();
        return false;
    }
    ...
    public override bool Valid(ushort command)
    {
        ...
        string errMsg = SharpVisionIntl.Get("File_Err_InvalidFileName", "Invalid file name.");
        if (owner != null)
