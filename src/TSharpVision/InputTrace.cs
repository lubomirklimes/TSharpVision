// InputTrace — opt-in input-routing diagnostics for TSharpVision.
//
// Enable by setting the environment variable:
//   TSHARPVISION_TRACE_INPUT=1
//
// Output is written to  tsharpvision-input-trace.log  in the CWD.
...
/// variable <c>TSHARPVISION_TRACE_INPUT=1</c>.
/// Output goes to <c>tsharpvision-input-trace.log</c> in the working directory.
/// </summary>
public static class InputTrace
{
    // ---- public API -------------------------------------------------------

    /// <summary>True when TSHARPVISION_TRACE_INPUT=1 was set at process start.</summary>
    public static readonly bool Enabled = 
        string.Equals(Environment.GetEnvironmentVariable("TSharpVision_TRACE_INPUT"),
    ...
    static InputTrace()
    {
        ...
        try
        {
            _writer = new StreamWriter(
                path: "TSharpVision-input-trace.log",
                append: false,
                encoding: Encoding.UTF8)
            { AutoFlush = true };
            _writer.WriteLine(
                $"[START] TSharpVision input trace — {DateTime.Now:yyyy-MM-dd HH:mm:ss}");
            _writer.WriteLine(
