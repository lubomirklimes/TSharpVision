// Migrated from SharpVision.Demo/Program.cs lines 66-126.
using SharpVision;
...
public sealed class NullDriverEventTests : IDisposable
{
    ...
    {
        var saved = Environment.GetEnvironmentVariable("SHARPVISION_DRIVER");
        try
        {
            Environment.SetEnvironmentVariable("SHARPVISION_DRIVER", "NullDriver");
            var d = ScreenDriverFactory.CreateScreenDriver();
            Assert.IsType<NullDriver>(d);
        }
        finally
        {
            Environment.SetEnvironmentVariable("SHARPVISION_DRIVER", saved);
        }
    }
