...
/// <summary>
/// Opens a SharpVision <c>.tvr</c> resource file, reads its index, and
...
public sealed class TvrInspector
{
    ...
    /// <exception cref="InvalidDataException">File is not a valid SharpVision <c>.tvr</c>.</exception>
    public static TvrInspector Open(string tvrPath)
    {
        ...
        try
        {
            ...
            if (rf.Count() == 0 && fileSize < 12)
                throw new InvalidDataException($"Not a valid SharpVision .tvr file: {tvrPath}");
